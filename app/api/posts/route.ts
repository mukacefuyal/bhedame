...
export async function GET(request: NextRequest) {
  ...
  const { data: posts, error } = await db.from("posts").select(publicPostColumns).order("created_at", { ascending: false }).limit(80);
  ...
  for (const reaction of reactions || []) {
    ...
    if (visitorId && reaction.visitor_id === visitorId) item.user = reaction.reaction as -1 | 1;
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!visitorId) return NextResponse.json({ error: "Missing visitor session. Refresh the page and try again." }, { status: 400 });
  ...
  try {
    // Burst protection plus a longer daily ceiling. Both IP and visitor ID are checked.
    const burst = await checkRateLimit(db, request, "create_post_10m", 4, 10 * 60, visitorId);
    if (!burst.allowed) return rateLimited(burst.retryAfter);
    const daily = await checkRateLimit(db, request, "create_post_day", 15, 24 * 60 * 60, visitorId);
  ...
  const publisherHash = publicPublisherHash(request);
  const since = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  const { data: duplicate, error: duplicateError } = await db
    .from("posts")
    .select("id")
    .eq("content_fingerprint", fingerprint)
    .eq("publisher_key_hash", publisherHash)
  ...
  const { data, error } = await db.from("posts").insert({
    ...
    author_name: authorName || "Guest sheep",
    content_fingerprint: fingerprint,
    publisher_key_hash: publisherHash,
