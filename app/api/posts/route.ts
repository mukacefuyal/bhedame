...
import { isSafeHttpUrl } from "@/lib/embed";
...
export async function GET(request: NextRequest) {
  ...
  const { data: posts, error } = await db.from("posts").select("*").order("created_at", { ascending: false }).limit(80);
...
export async function POST(request: NextRequest) {
  ...
  const embedUrl = body.embedUrl ? String(body.embedUrl) : null;
  ...
  if (["image", "video"].includes(mediaType) && !isSafeHttpUrl(mediaUrl)) return NextResponse.json({ error: "A valid uploaded media URL is required." }, { status: 400 });
  if (mediaType === "embed" && !isSafeHttpUrl(embedUrl)) return NextResponse.json({ error: "A valid embed URL is required." }, { status: 400 });
  ...
  const { data, error } = await db.from("posts").insert({
    ...
    embed_url: embedUrl, source_url: sourceUrl, author_name: authorName || "Guest sheep",
  }).select("*").single();
